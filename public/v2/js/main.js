...
document.getElementById("layout-menu") && (isHorizontalLayout = document.getElementById("layout-menu").classList.contains("menu-horizontal")), function () {
    ...
    var a, t = document.querySelector(".dropdown-style-switcher"),
        s = localStorage.getItem("templateCustomizer-" + templateName + "--Style") || (window.templateCustomizer?.settings?.defaultStyle ?? "light"),
        l = (window.templateCustomizer && t && ([].slice.call(t.children[1].querySelectorAll(".dropdown-item")).forEach(function (e) {
            ...
            title: "System Mode",
            fallbackPlacements: ["bottom"]
        }))), "system" === (a = s) && (a = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"), [].slice.call(document.querySelectorAll("[data-app-" + a + "-img]")).map(function (e) {
    ...
    if (l.length) {
        ...
        for (let e = 0; e < i.length; e++) i[e].addEventListener("click", function () {
            let o = this.getAttribute("data-language"), n = this.getAttribute("data-text-direction");
    ...
    n.length && n.on("click", function () {
        ...
        return function (t, e) {
            ...
            o = [], n.filter(function (e) {
                if (e.name.toLowerCase().startsWith(t.toLowerCase())) o.push(e); else {
