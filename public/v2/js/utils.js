...
utils.calculAge = ({dateNaiss}) =>{
    console.log(dateNaiss)
    var today = new Date();
    var dtn=dateNaiss; // on lit la date de naissance
    ...
    let month = m<0?m+12:m;
    return age+"an(s)"+month+"mois";
}
...
utils.datatable = ({
    ...
    $(`${element} tbody`).on("click", ".delete-record", function () {
...
utils.toast = ({
    message,
    title="",
    timeRelative = "maintenant",
    theme = 'info',
    position = 'top-2 end-2',
    duration = 3000,
    animation = "fade"
}) => {
    const newToast = $("#toast-template").clone();
    $('body').append(newToast);
    console.log("New Toast before: ", newToast, newToast.classList);
    newToast.removeAttr('id');
    newToast.find('#toast-title').text(title);
    newToast.find('#toast-time-relative').text(timeRelative);
    newToast.find('#toast-message').text(message);
    newToast.addClass(position).addClass("bg-"+theme).addClass("animate__"+animation);
    newToast.attr('data-bs-delay', duration)
    console.log("New Toast after: ", newToast, newToast.classList);
...
utils.formatNumber = ({num,localize,fixedDecimalLength}) => {
    num=num+"";
    var str=num;
